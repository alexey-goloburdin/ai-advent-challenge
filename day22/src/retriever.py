...
def _build_lookup(index: list[dict]) -> dict[tuple[str, int], dict]:
    ...
    """Индекс для быстрого доступа по (source, chunk_index)."""
    return {
...
def _expand_with_neighbors(
    ...
    """Собирает текст чанка + window соседей с каждой стороны."""
    source = chunk["source"]
...
def search(
    ...
    for record, score in scored[:top_k]:
        if window > 0:
            expanded_text = _expand_with_neighbors(record, lookup, window)
        else:

            expanded_text = record["text"]
