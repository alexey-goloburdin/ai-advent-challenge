...
def main() -> None:
    ...
    while True:
        try:
            ...
            if raw == r"\e":
                user_text = _read_from_editor().strip()

            elif raw.startswith("@"):
                user_text = _read_from_file_command(raw).strip()
        ...
        if stats is not None:
            prompt_tokens, completion_tokens, after_est = stats
            print(
                f"\x1b[3m"
                f"В контексте (input): {prompt_tokens}, "
                f"в ответе (output): {completion_tokens}, "
                f"после ответа (≈input+output): {after_est}"
                f"\x1b[0m"
            )
...
def _read_from_file_command(cmd: str) -> str:
    """
