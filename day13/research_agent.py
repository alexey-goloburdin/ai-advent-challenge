...
def call_llm(messages: list[dict], temperature: float = 0.7) -> str:
    ...
    req = urllib.request.Request(OPENAI_BASE_URL, data=data, headers=headers, method="POST")
...
class ResearchAgent:
    ...
    def _handle_clarification(self, user_input: str) -> str:
        """Обработка уточнений от пользователя."""
...
""")
    ...
    if state and state.phase != ResearchPhase.IDLE:
        ...
        if state.phase in (ResearchPhase.SEARCHING, ResearchPhase.ANALYZING, ResearchPhase.SYNTHESIZING):
            print(f"\n👉 Нажмите Enter для продолжения или /status для подробностей")
        elif state.phase == ResearchPhase.CLARIFICATION:
            print(f"\n👉 Ответьте на уточняющие вопросы выше или введите /status")
        elif state.phase == ResearchPhase.REVIEW:
            print(f"\n👉 Введите вопрос для углубления, /done для завершения или /export для сохранения")
        else:
