...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--model",               default="qwen/qwen3.5-9b",         help="Модель LM Studio")
    ...
    p.add_argument("--lm-studio-url",       default="http://localhost:1234",  help="LM Studio API URL")
    p.add_argument("--embed-model",         default="nomic-ai/nomic-embed-text-v1.5-GGUF", help="LM Studio модель для эмбеддингов")
...
def get_embedding(text: str, lm_studio_url: str, model: str) -> list[float]:
    ...
    url = lm_studio_url.rstrip("/") + "/v1/embeddings"
...
def main() -> None:
    ...
    for q in questions:
        ...
        if not args.no_plain:
            print("\n▶ Без RAG...")
            answer_plain = answer_without_rag(question, model=args.model, api_url=args.lm_studio_url)
